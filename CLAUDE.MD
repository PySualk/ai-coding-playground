...
1. **Frontend**: Angular 20.3.0 + Ionic 8.4.1 application with standalone components
   - Cyberpunk theme using Ionic CSS variables and theming system
   - See `frontend/CLAUDE.md` for details
...
3. **Kubernetes Deployment**: Production K8s manifests not configured
...
- Port forwarding: Frontend (:8100), Backend (:8080), Postgres (:5432)
...
```bash
cd frontend
npm install && npm start  # Starts on :8100
```
...
- **Ports**: Frontend :8100, Backend :8080, Postgres :5432, Tilt UI :10350
...
4. Capacitor for native mobile (iOS/Android)
